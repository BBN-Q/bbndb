    ...
    def create_default_pipeline(self, average=True, buffers=False):
        ...
        if average:
            if self.stream_type.lower() == "raw":
                self.add(Demodulate()).add(Integrate()).add(Average()).add(Output(groupname=self.qubit_name+'-raw'))
            if self.stream_type.lower() == "demodulated":
                self.add(Integrate()).add(Average()).add(Output(groupname=self.qubit_name+'-demod'))
            ...
            if self.stream_type.lower() == "raw":
                self.add(Demodulate()).add(Integrate()).add(Output(groupname=self.qubit_name+'-raw'))
            if self.stream_type.lower() == "demodulated":
                self.add(Integrate()).add(Output(groupname=self.qubit_name+'-demod'))
            if self.stream_type.lower() == "integrated":
